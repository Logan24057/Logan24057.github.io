 ...
 * @target MV MZ
 * 
 * @plugindesc [1.3.2] Custom Character Poses
 ...
 *      - This one is similar pretty similiar to the former one. The difference here is
...
Game_CharacterBase.prototype.charImgExists = function(file){
    if(TAA.cp.Parameters.Global.CheckFiles !== true)
        return true;
    if(!file || file === "" || ImageManager._invalidCharFiles.contains(file)){
        return false;
    }
    if(ImageManager.isCharCached(file)) return true;
    try{
        var http = new XMLHttpRequest();
        http.open('HEAD', "img/characters/" + file + ".png", false);
        http.send();
        if(http.status >= 400){
...
Game_CharacterBase.prototype.startBlink = function(){
    if(this._cpCurrentPose === undefined || this._cpPoseLibrary === undefined) return;
...
Game_CharacterBase.prototype.isBlinking = function(){
    if(this._cpCurrentPose === undefined || this._cpPoseLibrary === undefined) return false;
    var pose = this.currentPoseObj();
...
Game_CharacterBase.prototype.isIdleEnough = function(){
    if(!this.isPoseAvailable(this._cpIdlePose) || !this.isIdleAllowed()) return false;
    return !this.isIdlePose() && this._stopCount > this._cpIdleWaitTime;
};

Game_CharacterBase.prototype.shouldDash = function(){
    return (this.isMoving() && this.isDashing() && this.isPoseAvailable(this._cpDashPose));
};
...
Game_Player.prototype.initializePoses = function(){
    var actor = $gameParty.leader();
    if(actor && TAA.cp.Parameters.Actor[actor.actorId()] !== null){
...
Game_Follower.prototype.realMoveSpeed = function() {
    if(TAA.cp.Parameters.Global.Enable8Dir)
        ...
        return TAA.cp.alias.Game_Follower.realMoveSpeed.call(this);
};
...
Game_Event.prototype.loadPosesFromNotes = function(){
    ...
    if(notes.match(/<TAA_CP:\s*(?:,?noDefaults\s*|,?\s*default:[^\s<>,]+|,?\s*savePose:(?:false|true)|,?\s*dash:[^\s<>,]+|,?\s*idle:[^\s<>,]+|,?\s*lib:[^\s<>,]+(?:,\s*[^\s<>,]+)*|,?\s*balloon[s]?:[0-9]+(?:,\s*[0-9]+)*)+>/i)){
